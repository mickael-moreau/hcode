...
Parser.loadBoard = function(index, input, output) {
    ...
    if (0 === index) {
        ...
        Tools.assert(!isNaN(output.nb_columns), 'nb_columns should be a defined number');
        output.nb_drones = parseInt(tokens[2]);
        Tools.assert(!isNaN(output.nb_drones), 'nb_drones should be a defined number');
        output.nb_turns = parseInt(tokens[3]);
        Tools.assert(!isNaN(output.nb_turns), 'nb_turns should be a defined number');
        output.max_payload = parseInt(tokens[4]);
        Tools.assert(!isNaN(output.max_payload), 'max_payload should be a defined number');
    ...
    if (1 === index) {
        ...
        Tools.assert(!isNaN(output.nb_product_type), 'nb_product_type should be a defined number');
    ...
    if (2 === index) {
        ...
        for (var it = 0; it < tokens.length; it++) {
            ...
            Tools.assert(!isNaN(token), 'nb_product_type should be a defined number');
            output.weights_by_type.push(token);
        }
        Tools.assert(output.weights_by_type.length == output.nb_product_type,
    ...
    if (3 === index) {
        ...
        Tools.assert(!isNaN(output.nb_warehouses), 'nb_warehouses should be a defined number');
    ...
    if (index >= 4 && index <= warhouse_end_loading_sentinel) {
        if (0 === index % 2) {
            ...
            Tools.assert(!isNaN(warhouse.loc.x) && !isNaN(warhouse.loc.y)
    ...
    if (warhouse_end_loading_sentinel + 1 === index) {
        ...
        Tools.assert(!isNaN(output.nb_orders), 'nb_orders should be a defined number');
    ...
    && index <= order_end_loading_sentinel) {
        switch (Parser.order_modulo) {
            ...
            Tools.assert(!isNaN(order.loc.x) && !isNaN(order.loc.y)
            ...
            Tools.assert(!isNaN(nb_item), 'nb_item for order be a defined number');
